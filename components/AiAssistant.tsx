...
const AiAssistant: React.FC = () => {
  ...
  const getApiKey = () => {
    try {
      ...
      const viteKey = (import.meta as any).env?.VITE_AI_API_KEY as string | undefined;
      if (viteKey && viteKey !== 'undefined') return viteKey;

      // Fallback for older setups that used process.env.API_KEY
      if (typeof process !== 'undefined' && process.env && (process.env as any).API_KEY) {
  ...
  const handleSend = async (text: string) => {
    ...
    if (!API_KEY) {
      setMessages(prev => [...prev, { role: 'ai', text: "Neural Intelligence Core is offline. Please configure API_KEY." }]);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      const generateWithRetry = async () => {
        ...
        while (attempt <= maxRetries) {
          ...
          - Use bullet points for lists.`,
            ...
            const isRateLimit = status === 429 || /quota|rate limit|RESOURCE_EXHAUSTED/i.test(message);
            attempt += 1;
            if (attempt > maxRetries || !isRateLimit) break;
            const backoffMs = Math.pow(2, attempt) * 1000 + Math.floor(Math.random() * 300);
            console.warn(`GenAI rate limit detected; retrying in ${backoffMs}ms (attempt ${attempt}/${maxRetries})`);
      ...
      const aiText = response?.text || "I'm having trouble thinking right now. Please try again later!";
      ...
      try {
        if (error.response) {
          ...
          const data = error.response.data || error.response.body || JSON.stringify(error.response);
  ...
  return (
    <section id="ai-faq" className="py-24 bg-slate-50/50 dark:bg-slate-900/10 border-y border-slate-200/60 dark:border-slate-800/60">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold tracking-tighter mb-6">
            Interactive <span className="moveable-text animate-drift text-gradient">AI Insights</span>
          </h2>
        </div>
        ...
        <div className="bg-[#91a3b0] rounded-[3rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col h-[700px] transition-all relative">
          ...
          <div ref={scrollRef} className="flex-grow overflow-y-auto p-8 md:p-12 space-y-8 scroll-smooth scrollbar-hide relative z-10">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-12 animate-in fade-in duration-1000">
                ...
                <div className="space-y-6 max-w-sm">
                  <p className="text-2xl font-black text-white tracking-tight">How may I assist you today?</p>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-6 duration-500`}>
                ...
                <div className={`max-w-[90%] px-8 py-6 rounded-[2.5rem] text-[15px] font-medium leading-relaxed shadow-sm ${
                  msg.role === 'user' 
            ...
            {isLoading && (
              <div className="flex flex-col items-start animate-in fade-in duration-300">
                <span className="text-[9px] font-black uppercase tracking-widest mb-2 text-white/50 px-2">Processing...</span>
          ...
          <div className="p-8 bg-slate-950/20 backdrop-blur-md border-t border-white/10 relative z-10">
            <form onSubmit={(e) => { e.preventDefault(); handleSend(input); }} className="flex gap-4">
              <input 
                type="text" 
                value={input} 
