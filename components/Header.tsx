  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div
          className={`transition-all duration-500 border rounded-[2rem] px-10 flex justify-between items-center ${
            scrolled
        ...
        >
          <a href="#home" className="flex items-center gap-4 group">
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-500 group-hover:rotate-[360deg] shadow-lg overflow-hidden ${
          ...
          <nav className="hidden md:flex items-center space-x-14">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span
                  className={`absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-1 rounded-full transition-all group-hover:w-6 ${
      ...
      {/* Mobile Overlay */}
      <div
        className={`md:hidden fixed inset-0 bg-white dark:bg-slate-950 z-[90] transition-transform duration-700 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="flex flex-col items-center justify-center h-full space-y-12">
          {navLinks.map((link) => (
            <a
              ...
              className="text-5xl font-black uppercase tracking-tighter text-slate-900 dark:text-white"
            >
              {link.name}
            </a>
          ))}
          <a
            href="#contact"
            onClick={() => setIsOpen(false)}
            className="px-14 py-6 bg-primary-600 text-white rounded-[2rem] text-lg font-black uppercase tracking-widest"
          >
            Connect
          </a>
        </div>
