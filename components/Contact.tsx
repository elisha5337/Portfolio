...
const Contact: React.FC = () => {
  ...
  const getEnv = (key: string) => {
    try {
      ...
      const viteVal = viteKey ? ((import.meta as any).env?.[viteKey] as string | undefined) : undefined;
  ...
  const socialLinks = [
    { name: 'Facebook', url: 'https://web.facebook.com/?_rdc=1&_rdr#', icon: 'FB' },
    { name: 'Instagram', url: 'https://www.instagram.com/elishaarba/', icon: 'IG' },
    { name: 'Telegram', url: 'https://t.me/onetruthlife', icon: 'TG' },
    { name: 'TikTok', url: 'https://www.tiktok.com/@elishaelegant', icon: 'TT' }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
          <div className="flex flex-col justify-center">
            <h2 className="text-sm font-bold text-[#91a3b0] uppercase tracking-[0.3em] mb-4">Connect</h2>
            <h3 className="text-5xl md:text-6xl font-extrabold mb-8 leading-tight tracking-tight text-slate-900 dark:text-white">
              Let's Build <span className="topic-accent animate-float-subtle">Something</span> <br /> 
              <span className="bg-gradient-to-r from-[#91a3b0] to-primary-500 bg-clip-text text-transparent underline decoration-[#91a3b0]/20 topic-accent animate-drift-minimal">Extraordinary.</span>
            </h3>

            <p className="text-xl text-slate-500 dark:text-slate-400 mb-12 font-medium leading-relaxed max-w-lg">
              Currently accepting <span className="text-[#91a3b0]">new projects</span> and consulting engagements. Reach out via the form or direct channels.
            </p>
            ...
            <div className="space-y-6 mb-16">
              <div className="flex items-center gap-6 group">
                ...
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Direct Email</p>
                  <a href="mailto:elsayearba@gmail.com" className="text-xl font-bold text-slate-900 dark:text-white hover:text-[#91a3b0] transition-colors tracking-tight">elsayearba@gmail.com</a>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6 group">
                ...
                <div>
                  ...
                  <a href="tel:0924274666" className="text-xl font-bold text-slate-900 dark:text-white hover:text-[#91a3b0] transition-colors tracking-tight">0924 274 666</a>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Your Name</label>
                    <input 
                      type="text" 
                      required
                  ...
                  <div className="space-y-3">
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Email Address</label>
                    <input 
                      type="email" 
                      required
                ...
                <div className="space-y-3">
                  <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Project Vision</label>
                  <textarea 
                    required
                    rows={6}
              ...
              <div className="space-y-4">
                <button 
                  ...
                  className={`w-full py-6 rounded-2xl font-black text-sm uppercase tracking-[0.3em] transition-all shadow-2xl disabled:opacity-50 flex items-center justify-center gap-4 active:scale-[0.98] ${
                    status === 'error' ? 'bg-red-600 shadow-red-500/20' : 'bg-white hover:bg-slate-100 shadow-white/20 text-slate-900'
                  }`}
                >
                  {status === 'sending' ? 'Transmitting...' : status === 'sent' ? 'Success!' : status === 'error' ? 'Error' : 'Send Message'}
                </button>
              </div>
            </form>
          </div>
