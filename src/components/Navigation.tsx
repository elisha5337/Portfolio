...
export const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-panel/90 backdrop-blur-xl px-8 py-4 border-b border-border shadow-2xl' : 'bg-transparent px-8 py-6'}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <a href="#" onClick={scrollToTop} className="flex items-center gap-3 group">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight text-current uppercase leading-none">{PERSONAL_INFO.name}</h1>
            <p className="text-[10px] text-accent uppercase tracking-widest font-bold opacity-80">{PERSONAL_INFO.profession}</p>
          </div>
        </a>
        ...
        <div className="md:hidden flex items-center gap-4">
          <button onClick={toggleDarkMode} className="p-2 rounded border border-border">
            {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 text-accent">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            <button onClick={() => setIsMenuOpen(false)} className="absolute top-8 right-8 p-2 text-slate-500">
               <X className="w-6 h-6" />
            </button>
